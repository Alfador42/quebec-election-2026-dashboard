...
test('repeated production submissions do not change historical averages or event count',()=>{
  const copy=structuredClone(state),before=buildDataset(copy,'2026-09-12');
  for(const event of state.events) {
    const p=event.revisions.find(r=>r.id===event.activeRevisionId).poll;
    assert.equal(ingest(copy,p,{now:'2026-09-12T23:59:00Z'}).kind,'duplicate');
  }
  const after=buildDataset(copy,'2026-09-12');
